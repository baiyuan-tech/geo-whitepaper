...
- **DOI(v1.3.0,current)**:[`10.5281/zenodo.22105080`](https://doi.org/10.5281/zenodo.22105080)
- **DOI(v1.2.0)**:[`10.5281/zenodo.21222575`](https://doi.org/10.5281/zenodo.21222575)
...
- [Ch 21 — 客戶自有網域的 AXP 交付:CF Worker 與子網域 CNAME](zh-TW/ch21-axp-delivery-modes.md) **(v1.3 新增)**
...
- **v1.3.0 Version DOI(current)**:[`10.5281/zenodo.22105080`](https://doi.org/10.5281/zenodo.22105080)
- **v1.2.0 Version DOI**:[`10.5281/zenodo.21222575`](https://doi.org/10.5281/zenodo.21222575)
...
| **2026-08-26** | **v1.3.0** | **新增 Ch 20(掃描引擎的程式化供給)+ Ch 21(客戶自有網域的 AXP 交付)**。**Ch 20**:把既有多平台掃描引擎以 pull-based executor 對外供給 —— 不開對內攻擊面(executor 而非 endpoint)/ pass-through 執行 / 批發 track 五維隔離不污染核心資料 / exactly-once 交付狀態機(冪等 + `submitting` in-doubt + 計費補償 sweep)/ 誠實記帳(成本與服務費脫鉤,cache 與失敗不計)/ 有界並發 `p-limit(N)` / 零基建市場視角(提示層注入取代地區 IP,並顯式區分「語言指示 ≠ 市場視角」誠實界線)。**Ch 21**:交付無關的 backend 支撐多種 AXP 交付模式 —— CF DNS + CF Worker(委派網域)/ Tier A 子網域 CNAME(Cloudflare for SaaS,只加 1 筆 CNAME 不委派)/ delivery_mode 互斥 SSOT + 先建新再拆舊安全切換 / 自訂主機名 SSL 狀態機 / 方案 gating(子網域 CNAME Pro+、CF DNS Enterprise+),含各模式請求資料流圖。zh-TW + en + ja 三語同步(自此三語皆涵蓋 Ch 1-21 全篇);mermaid 資料流圖首次內嵌進 PDF。Concept DOI [`10.5281/zenodo.19994035`](https://doi.org/10.5281/zenodo.19994035) 不變;v1.3.0 Version DOI [`10.5281/zenodo.22105080`](https://doi.org/10.5281/zenodo.22105080) 已由 Zenodo 分配 |
