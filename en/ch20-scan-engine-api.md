...
- **Fidelity boundary**: an API response may differ from the "real web-UI answer" (e.g., with or without search, logged-in or not). Text-only supply does no screenshots / browser automation — a deliberate scope trade-off the consumer must know and accept.
