...
> **ブランドレベル分離（2026-04-21 更新）**：テナント内に複数ブランドが存在する場合、各ブランドはさらに専用の Knowledge Base ID（`rag_kb_id`）によって知識範囲を分離される。AXP 生成時はブランドの `kbId` を指定してクエリを実行し、同一テナント内の異なるブランド（例：コスメブランドとフードブランド）の知識が混在しないことを保証する。新ブランドが AXP を有効化すると、`seedBrandRAGKB` が自動的に KB を作成してブランドプロフィールと公式サイト URL をアップロードする——詳細は [§6.10](./ch06-axp-shadow-doc.md#610-rag-知識ベース連携ブランド専用-kb-の自動化) を参照。
...
ClaimReviewはSchema.org標準プロパティ — Google、Facebook、Twitterすべてがパースする。百元独自の発明ではなく、グローバルなファクトチェック生態系の相互運用可能な構成要素である。[^claimreview][^factcheck]
