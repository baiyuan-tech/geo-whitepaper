...
- **忠実度の境界**:API 応答は「実在の web UI の答え」と異なりうる(検索の有無、ログイン状態の有無など)。本方式は**実在の AI web UI をスクリーンショットしない** —— それはログイン状態の維持と反自動化への対抗を要し、かつ web 版は別の生成で API の答えと一致しないからである。**オプション(デフォルトはオフ)**として、供給側は**自分が取得した API の答え**をサーバー側で「アンサーカード」PNG にレンダリング(`satori` → PNG、ブラウザなし)して返せる;消費者はそれを引用エビデンスカードとして使える。これは**忠実度の境界を変えない** —— 提示されるのは依然として API のプレーンテキストの答えそのものであり、視覚的な載体が変わるだけである。「アンサーカードのレンダリング ≠ 実在 UI のスクリーンショット」という区別は消費者が知り受け入れるべきである。
