...
- [13.8 v1.2 增補:VideoObject 完整化、same-origin filter 與多模態 sitemap extension](#138-v12-增補videoobject-完整化same-origin-filter-與多模態-sitemap-extension)
- [13.9 已知限制與 v0.2 規劃](#139-已知限制與-v02-規劃)
- [本章要點](#本章要點)
- [參考資料](#參考資料)
...
## v1.2 增補:VideoObject 完整化、same-origin filter 與多模態 sitemap extension

本節記錄 v1.1 之後(2026 年中)在多模態 GEO 上補齊的三項工程,對齊 Google 對圖片 / 影片結構化資料的規範。

### VideoObject 完整 GSC 規格與 origin 倒灌

v1.1 的 VideoObject 只帶基本欄位,常以品牌名 fallback(例「品牌 影片資產」)。補齊後對齊 Google video 規範:`thumbnailUrl`(`maxresdefault` / `hqdefault` 兩種 quality fallback)、`uploadDate`、`duration`(ISO 8601)、`transcript`、`publisher` / `about` / `creator` 關聯回品牌實體(`#brand-{id}` anchor),並補上與 ImageObject 同一套的四個 GSC 授權欄位(`creditText` / `copyrightNotice` / `acquireLicensePage` / `license`)。

關鍵是**零接觸倒灌**:客戶 origin 頁若已有 YouTube / Vimeo `<iframe>` 且頁面內含 inline `VideoObject` JSON-LD,visualCrawler 會 flatten 其 `@graph` 並以 `youtube_id` / `contentUrl` / `embedUrl` 反查,把 `name` / `description` / `transcript` / `uploadDate` / `duration` 倒灌進 `visual_assets`(不覆蓋既有真實值)。任何客戶用 Next.js / WordPress / Shopify SSR 自帶的 VideoObject 都被自動拾取,無需改 theme。資料庫為此加了 `upload_date DATE` 與 `duration TEXT` 兩欄。

一個踩過的雷:`transcript` 欄位一度被遺漏在 upsert 的 `ON CONFLICT` 子句,導致任何 brand re-crawl 都丟失 transcript;修補後 `ON CONFLICT` 全欄位 `COALESCE` 不丟值。

### same-origin filter:不偽稱外部圖片版權

平台替客戶注入的 ImageObject / VideoObject 帶了 `creator` / `copyrightHolder = 品牌`。若該圖其實是客戶頁面引用的**外部域名圖片**(第三方素材、CDN、tracker pixel),平台就是在替客戶宣稱一張不屬於他的圖的版權 — 這是 false copyright claim,會被 GSC 拒收並降低 AI 信任度。

三層防護:

1. **抓取階段 same-origin filter** — `isInternalImage(url, brand.website)` 只把與 origin 同源的圖寫入 `visual_assets`,外部域名圖不寫入(防新污染)。
2. **歷史 row 清理** — 對既有外部圖 row 設 `schema_jsonld = NULL`。
3. **輸出過濾** — sitemap `<image:image>` 只 emit `schema_jsonld IS NOT NULL` 的 row。

同時,tracker URL(google-analytics、googletagmanager、doubleclick、hotjar、criteo 等)一律 reject,不進 schema。原則是:**平台只替客戶自家 origin 真實擁有的視覺資產背書;對外部圖片的 schema 一律留空** — 寧可少注,不可偽稱。

### sitemap 影像與影片 extension

inline schema 之外,sitemap 的 extension 讓 Google 更快索引視覺資產。兩個 Google 規範的硬性細節(皆踩過雷):

- `<image:image>` 出現時,sitemap 根元素**必須**宣告 `xmlns:image` namespace,否則 Google 直接 reject 整份 sitemap;`<video:video>` 同理需 `xmlns:video`。
- `<video:publication_date>` 必須 ISO 8601 日期(`YYYY-MM-DD`);直接 `String(new Date())` 會輸出 `Wed May 06 2026...` 被 Google 拒收整份 sitemap。`<video:duration>` 必須是秒數整數(規範 1–28800 範圍),不可吐 ISO 8601 `PT2M30S` 字串。兩者由專用 helper 規範化。

per-URL 保守上限(image 30 / video 5)避免大型電商頁的 sitemap 膨脹。此鏈與 [Ch 18 — AXP HTML Mirror-First](./ch18-axp-html-mirror-first.md)的 sanitizer 設計呼應:HTML 本體禁 `<iframe>` / `<video>`(XSS 防護),影片改走此處的 VideoObject schema 與 sitemap video extension 呈現。傳播時效由 [Ch 19 — 快取失效 5 層架構](./ch19-cache-invalidation.md)保證。
...
**導覽**:[← Ch 12: 限制與未來](./ch12-limitations.md) · [📖 目次](../README.md) · [Ch 14: F12 三層結構優化器 →](./ch14-f12-structural-optimizer.md)
