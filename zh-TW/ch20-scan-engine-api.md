...
last_modified_at: '2026-08-30T14:35:41+08:00'
---
...
- **保真度界線**:API 應答與「真實網頁 UI 答案」可能不同(如帶不帶搜尋、有無登入態)。本方案**不截真實 AI 網頁 UI** —— 那需維護登入態並對抗反自動化,且網頁版是另一次生成、與 API 答案對不上。作為**可選項(預設關)**,供給端可把**我方拿到的 API 答案**在伺服器端渲染成一張「答案卡」PNG(`satori` → PNG,不啟動瀏覽器)回推,供消費方當引用證據卡;它**不改變保真度界線** —— 呈現的仍是 API 純文字答案本身,只是換個視覺載體。「渲染答案卡 ≠ 截真實 UI」這條區分需消費方知悉並接受。
